...
import RegisterRoomPhotoCardList from '../../room/register/RegisterRoomPhotoCardList';
  ...
  const uploadImage = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      try {
        ...
        if (data) dispatch(registerRoomActions.setPhotos([...photos, data]));
      } catch (e) {
        console.error(e);
      }
    }
      ...
      {isEmpty(photos) && (
        <div className="register-room-upload-photo-wrapper">
          <>
            ...
            <Button color="bittersweet" width="167px" icon={UploadIcon}>
              사진 업로드
            </Button>
          </>
        </div>
